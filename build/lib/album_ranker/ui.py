        ...
        button.addEventListener("click", async (event) => {{
          ...
          try {{
            ...
            if (payload.bookmarked_at && !payload.listened_at) {{
              const openIds = [...document.querySelectorAll(".list-block")]
                .filter((b) => !document.getElementById("list-body-" + b.dataset.listId)?.classList.contains("hidden"))
                .map((b) => b.dataset.listId);
              if (openIds.length) sessionStorage.setItem("openListIds", JSON.stringify(openIds));
